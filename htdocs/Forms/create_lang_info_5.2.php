...
//allows user to select syllible onset allowed/forbidden consonant clusters 
...
<html>
    <head>
        <meta charset="UTF-8">
        <title>Create_language_info_clusters_final</title>
...
<script>
	...
	$( "#space" ).on( "click", function() {
			...
			//if the character at the end of this string is + it gets sliced off
			...
			var space = " ";
			//selected_v stands for seleced values
	...
	$( "#back" ).on( "click", function() {
			...
			//if the character at the end of the text area is a +, 2 characters are sliced off. 
			if (val.charAt(val.length-1) == "+") {
				txt.text( txt.text().slice(0,-2) );
			}
			else {
				//if the character at the end of the text area is not a +, 1 character is sliced off
				txt.text( txt.text().slice(0,-1) );
			}
			
			//if the character at the end of the hidden text area is blank, 1 character is sliced off. 
			if (val_2.charAt(val_2.length-1) == " " || val_2.charAt(val_2.length-1) == "!" || val_2.charAt(val_2.length-1) == "*" ) {
				txt_2.text( txt_2.text().slice(0,-1) );
			}
			//if the character at the end of the hidden text area is a + and the second to last character is not *, 5 characters are sliced off. 
			else if (val_2.charAt(val_2.length-1) == "+" && val_2.charAt(val_2.length-2) != "*") {
				txt_2.text( txt_2.text().slice(0,-5) );
			}
			//if the character at the end of the hidden text area is not a +, 4 characters are sliced off
			else {
				txt_2.text( txt_2.text().slice(0,-4) );
			}
	...
	//appends a ! to represent all consonants
	...
	<div class="tooltip">Help
		<span class="tooltiptext">
			<p>This form allows you to create syllable coda consontant clusters from the syllable coda consonants that you already selected.</p>
			...
			<p>You can use the all consonants button to create a lot of clusters simultaniously. An example of this would be if you wanted /k/ to be able to proceed all other
			previously defined consonants. This would look like: k+*.</p>
			<p>You can make exceptions to your all consonant rule by using the exept button. If you wanted to a rule stating that /k/ can be followed all consonants except for /d/ 
			it would look like this: k+*+!d.</p>
	...
	<br>
	<form id="info_form" name="info_form" action="../processing/process_lang_info_5.2.php" method="post">
	<div id="text_div" style="font-size:80%">
        
		
<p style="font-size:120%">Choose <span style="color:red">allowed</span> syllable <span style="color:red">coda</span> consonant clusters </p>
		...
		<table>
			...
			<tr>
				...
				<?php
					...
					foreach ($_SESSION['final_consonants'] as $key => $value) {
...
$(document).ready(function() {	
	...
	$( "#<?php echo $key;?>" ).on( "click", function() {
			...
			<div class="wrapper">
				...
				<br>
				<br>
				<button type="button" id="all_c" id="all_c" name="all_c" value="*" align = "center">All Consonants</button>
